locals {
  subnet_keys = [for key, _ in var.subnet_private : key]
}
...
resource "aws_nat_gateway" "ngw" {
  for_each = { for idx, subnet in var.subnet_public : idx => subnet }
...
resource "aws_route_table" "rt_public" {
  ...
  tags = {
    "Name"                      = "public-${var.region}-${tonumber(replace(each.key, "_", "")) % length(var.availability_zones)}"
...
resource "aws_subnet" "subnet_public" {
  ...
  availability_zone = element(var.availability_zones, tonumber(replace(each.key, "_", "")) % length(var.availability_zones))
...
resource "aws_subnet" "subnet_private" {
  ...
  tags = {
    "Name"                      = "private-${var.region}-${tonumber(replace(each.key, "_", "")) % length(var.availability_zones)}"
    "kubernetes.io/cluster/eks" = "shared"
    "kubernetes.io/role/elb"    = "1"
  }
}
...
resource "aws_route_table_association" "rta_private" {
  for_each = { for idx, subnet in aws_subnet.subnet_private : idx => subnet }
